...
import com.dicoding.todoapp.ui.list.TaskTitleView.Companion.DONE
import com.dicoding.todoapp.ui.list.TaskTitleView.Companion.NORMAL
import com.dicoding.todoapp.ui.list.TaskTitleView.Companion.OVERDUE
...
class TaskAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TaskViewHolder {
        val binding = LayoutInflater.from(parent.context).inflate(R.layout.task_item, parent, false)
        return TaskViewHolder(binding)
    }

    override fun onBindViewHolder(holder: TaskViewHolder, position: Int) {
        val task = getItem(position) as Task
        //TODO 9 : Bind data to ViewHolder (You can run app to check) -Done
        holder.bind(task)
        when {
            //TODO 10 : Display title based on status using TitleTextView -Done
            task.isCompleted -> {
                //DONE
